...
class Spp_approved extends REST_Controller {
    ...
    function get_summary_spp_post()
    {
        if(VALID_LOGIN === true){
            ...
            $result= $this->Spp_model->getDataSppBendahara();
            $dataArr = array();
            $dataArrField = array();
            if(!empty($result)){
                foreach ( $result as $data ){
                    $detailBidangIdr = $this->Spp_model->get_ttl_realiization($data->id);
                    $getBankDetial = $this->Spp_model->db->select('name')->get_where('ref_bank',array('id'=>$data->rek_bank_id))->result_array()[0];
                    // print_r($getBankDetial['name']);die();
                    
                    // print_r($detailBidangIdr->ttl_realization);die();
                    ...
                    $array_detail['doc_si_id']     = $data->doc_si_id;
                    $array_detail['doc_bt_id']     = $data->doc_bt_id;
                    
                    $array_detail['transfer_date']     = $data->transfer_date;
                    $array_detail['receipt_number']     = $data->receipt_number;
                    $array_detail['bank_name']          = (!empty($data->rek_bank_id)) ? $getBankDetial['name'] : '-';
                    $array_detail['rek_num']     = $data->rek_num;
                    $array_detail['rek_name']     = $data->rek_name;
                    $array_detail['ttl_realization'] = $this->general_library->format_rupiah($detailBidangIdr->ttl_realization);
        ...
        if(VALID_LOGIN === true){
            ...
            if(!empty($result)){
                foreach ( $result as $data ){
                    ...
                    $array_detail['date_received']   = convert_date($data->date_received);
                    array_push($dataArr,$array_detail);
                }
                $this->response(array(
                    'status'=>'success',
