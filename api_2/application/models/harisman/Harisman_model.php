...
class Harisman_model extends MY_Model {
    ...
    public function getMainMenuAbsen($data)
    {
        // print_r($data['mgrupuser']);die();
        $this->db->select('*');
        $this->db->from('menu_side');
        $this->db->where('is_main_menu',0);
        $this->db->where('stat',1);
        $this->db->where('is_absen',1);
        $this->db->like("role_wfh",$data['mgrupuser']);
        $this->db->order_by('init');
        // $this->db->where("password",$data['password']);
        $query = $this->db->get();
        // $result = $query->num_rows();
    ...
    public function getSubMenuAbsen($data)
    {
        // print_r($data['mgrupuser']);die();
        $this->db->select('*');
        $this->db->from('menu_side');
    ...
    public function getDataPegawaiByNama()
    {
        $data = $_POST;
        $this->db->select('pegawai.id,pegawai.nip_npp,pegawai.nama,pegawai.nama_panggilan,pegawai.jenis_kelamin');
        $this->db->select('ref_jk.uraian');
        $this->db->join('ref_jk','pegawai.jenis_kelamin = ref_jk.id','LEFT');
        $this->db->from('pegawai');
        $this->db->like('pegawai.nama',$data['namauser']);
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    ...
    //get data seminar
    public function get_data_seminar()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
            if($data['urutane'] != ''){
                $where['riwayat_seminar.id_uniq']= $data['urutane'];
            }
        }
        $this->db->select('riwayat_seminar.*');
        $this->db->select('ref_negara.name as negara_name');
        $this->db->select('ref_peran_seminar.uraian as peran_seminar_uraian');
        // $this->db->select('ref_ijin_belajar.uraian as ijin_belajar');
        // $this->db->select('ref_jenjang_pendidikan.jenjang as pendidikan');
        $this->db->from('riwayat_seminar');
        $this->db->join('ref_negara','ref_negara.id=riwayat_seminar.id_negara_seminar','LEFT');
        $this->db->join('ref_peran_seminar','ref_peran_seminar.id=riwayat_seminar.id_peran_seminar','LEFT');
        // $this->db->join('ref_ijin_belajar','ref_ijin_belajar.id=riwayat_seminar.id_ijin_belajar','LEFT');
        // $this->db->join('ref_jenjang_pendidikan','ref_jenjang_pendidikan.id=riwayat_seminar.id_pendidikan','LEFT');
        $this->db->where($where);
        $this->db->where('riwayat_seminar.status',1);
        $query =  $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    ...
    //menu status
    //select status
    public function get_sp_select_status()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
        ...
        $this->db->select('id as urutane,uraian as jenenge');
        $this->db->from('ref_status_pegawai');
        $this->db->where($where);
    ...
    public function get_data_status()
    {
        ...
        if(isset($data['urutane'])){
            if($data['urutane'] != ''){
                $where['riwayat_status_pegawai.id_uniq']= $data['urutane'];
            }
        }
        $this->db->select('riwayat_status_pegawai.*');
        $this->db->select('ref_status_pegawai.uraian as nm_status');
        $this->db->from('riwayat_status_pegawai');
        $this->db->join('ref_status_pegawai','ref_status_pegawai.id=riwayat_status_pegawai.id_status_pegawai','LEFT');
        $this->db->where($where);
        $this->db->where('riwayat_status_pegawai.status',1);
        $query =  $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    ...
    //karya tulis
    public function get_kt_select_jenispublikasi()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
        ...
        $this->db->select('id as urutane,uraian as jenenge');
        $this->db->from('ref_jns_publikasi');
        $this->db->where($where);
        $this->db->where('is_disable',0);
        $query =  $this->db->get();
        $result = $query->result_array();
    ...
    public function get_data_karyatulis()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
            if($data['urutane'] != ''){
                $where['riwayat_karyatulis.id_uniq']= $data['urutane'];
            }
        }
        $this->db->select('riwayat_karyatulis.*');
        $this->db->select('ref_jns_publikasi.uraian as nm_publikasi');
        $this->db->from('riwayat_karyatulis');
        $this->db->join('ref_jns_publikasi','ref_jns_publikasi.id=riwayat_karyatulis.id_jns_publikasi','LEFT');
        $this->db->where($where);
        $this->db->where('riwayat_karyatulis.status',1);
        $query =  $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    ...
    //penghargaan
    public function get_kt_select_jenispenghargaan()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
        ...
        $this->db->select('id as urutane,uraian as jenenge');
        $this->db->from('ref_jns_penghargaan');
        $this->db->where($where);
        $this->db->where('is_disable',0);
        $query =  $this->db->get();
        $result = $query->result_array();
    ...
    public function get_data_penghargaan()
    {
        $data =$_POST;
        $where = array();
        if(isset($data['urutane'])){
            if($data['urutane'] != ''){
                $where['riwayat_penghargaan.id_uniq']= $data['urutane'];
            }
        }
        $this->db->select('riwayat_penghargaan.*');
        $this->db->select('ref_jns_penghargaan.uraian as nm_penghargaan');
        $this->db->from('riwayat_penghargaan');
        $this->db->join('ref_jns_penghargaan','ref_jns_penghargaan.id=riwayat_penghargaan.id_jns_penghargaan','LEFT');
        $this->db->where($where);
        $this->db->where('riwayat_penghargaan.status',1);
        $query =  $this->db->get();
        $result = $query->result_array();
        return $result;
    }
