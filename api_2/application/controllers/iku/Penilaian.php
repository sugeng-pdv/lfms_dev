...
class Penilaian extends REST_Controller {
    ...
    function search_post() {
      ...
      if (isset($postdata)) {
        ...
        $result= $this->Penilaian_model->getData($postdata);
      } else {
        // echo "..,,,,,,";die();
        $result= $this->Penilaian_model->get_all();
      }
    ...
    function index_get() {
      ...
      if ($id == '') {
        ...
        // echo "test";die();
              // $where['nip'] = $id;
              // $result= $this->Penilaian_model->getData($where);
              $result= $this->Penilaian_model->get(array('id_p'=>$id));
      }
      // echo $result;die();
      $this->response($result, 200);
    }
    ...
    function index_post() {
        ...
        if($this->form_validation->run('perspective_post') != false){
            ...
            // print_r($data);die();
            // $safe_data = $this->Penilaian_model->get(array('id_per'=>"11"));
            $safe_data = $this->Penilaian_model->get(array('nm_pers'=>$this->post('nm_pers'),'tahun'=>$this->post('tahun')));
            if(!empty($safe_data)){
              // echo "string";die();
            ...
            $data_id = $this->Penilaian_model->insert($data);
            if (!$data_id){
              // echo "string";die();
                $this->response( array('status'=>'failure',
    ...
    function save_edit_post() {
        ...
        if($this->form_validation->run('perspective_post') != false){
            ...
            $safe_data = $this->Penilaian_model->get(array('id_per'=>$this->post('id_per')));
            if(!isset($safe_data)){
                $this->response( array('status'=>'failure',
                'message'=>'the specified no data to update',REST_Controller::HTTP_CONFLICT));
            }

            $data_id = $this->Penilaian_model->update($data,array('id_per'=>$this->post('id_per')));
            if (!$data_id){
                $this->response( array('status'=>'failure',
                'message'=>$this->form_validation->get_errors_as_array()),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
            } else {
    ...
    function index_delete() {
        $id = $this->uri->segment(2);
        $this->load->model('Penilaian_model');
        $data = $this->Penilaian_model->get(array('id_per'=>$this->delete('id_per')));
        if (isset($data)){
            $deleted = $this->Penilaian_model->force_delete(array('id_per'=>$this->delete('id_per')));
            if (!$deleted){
                $this->response( array('status'=>'failure',
