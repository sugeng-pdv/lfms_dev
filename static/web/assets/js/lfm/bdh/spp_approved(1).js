            ...
            { 
                ...
                if(resp.status === 'success'){
                    $.each(dataColums, function( index, value ) {
                        ...
                        $('#spp-'+value.id+' #btnCompleteField').attr('onclick','detailBidang("'+value.id+'","'+value.spp_no+'","'+value.psn_name+'","'+value.status_spp+'","'+value.nominal_idr+'","'+value.psn_type+'","'+value.payment_to+'")');
                        $('#spp-'+value.id).show();
           			});
...
function detailBidang(id,spp,name,status,nominal,payment,payment_to)
{
    ...
    window.localStorage.setItem('spp_status','Dalam Proses Penelitian');
